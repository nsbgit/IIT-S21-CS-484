...
purchaseLikelihood = pd.read_csv('D:\\IIT Edu\\Sem1\\MachineLearning\\Week10_assignment4\\Purchase_Likelihood.csv', delimiter=',',
...
purchaseLikelihood = pd.read_csv('D:\\IIT Edu\\Sem1\\MachineLearning\\Week10_assignment4\\Purchase_Likelihood.csv')

print("#########################################Question 2a ########################################")
countTarget = pd.DataFrame(columns = ['Count', 'Class_probability'])
countTarget['Count'] = purchaseLikelihood['A'].value_counts()
countTarget['Class_probability'] = purchaseLikelihood['A'].value_counts(normalize=True)
print(countTarget)
...
for col in purchaseLikelihood.columns[:-1]:
    crossTabulation(purchaseLikelihood['A'],purchaseLikelihood[col],'ROW')
...
for col in purchaseLikelihood.columns[:-1]:
    chiSqStat, chiSqDf, chiSqSig, cramerV = ChiSquareTest(purchaseLikelihood[col], purchaseLikelihood['A'], debug = 'Y')
...
yTrain = purchaseLikelihood['A'].astype('category')
