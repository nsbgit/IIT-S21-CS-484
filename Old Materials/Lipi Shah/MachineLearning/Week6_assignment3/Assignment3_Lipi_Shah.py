...
#print('Number of Observations in Training = ', X_train.shape[0])
#print('Number of Observations in Testing = ', X_test.shape[0])
index = ['Row'+str(i) for i in range(1, len(y_train)+1)]
df_y_train = pd.DataFrame(y_train, index = index)
#print("Count on Train Target Data ")
#print(df_y_train.groupby([0]).size())
#print("Proportion on Train Target Data")
#print(df_y_train.groupby([0]).size() / y_train.shape[0])
...
#print("Count on Train Target Data ")
#print(df_y_test.groupby([0]).size())
#print("Proportion on Train Target Data")
#print(df_y_test.groupby([0]).size() / y_test.shape[0])
...
#create data frame with required column attributes
claim_history2 = claim_history[["CAR_TYPE", "OCCUPATION", "EDUCATION", "CAR_USE"]].dropna()

#mapping Education category with [0,1,2,3,4] respectively
...
#)The Kolmogorov-Smirnov statistic and the event probability cutoff value
# Draw the Kolmogorov Smirnov curve
#fpr, tpr, thresholds = metrics.roc_curve(test_data_y, predProb_y, pos_label = 'Event')
...
#plt.show()
#print('################## Answer 3 A ###########################################')
#misclasssification rate of Test Partition
...
#    if predProb_y_train[i] > threshold:
